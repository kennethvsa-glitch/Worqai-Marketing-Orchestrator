...
### 6.3 S1 HOOK FORMULA — every first slide must pass this

**The formula:** `[Object] + [Problem] + [Mechanism]`

Every first slide headline must contain all three:
- **Object** — what the carousel is about (CV, ATS, LinkedIn, currículum, hoja de vida, perfil)
- **Problem** — what goes wrong (no pasa, no lee, ignora, descarta, rechaza, falla)
- **Mechanism** — why / the enemy (ATS, filtro, reclutador, sistema, algoritmo)

| Type | Example |
|------|---------|
| ✗ Vague (no object, no mechanism) | "3 cambios que duplican tus entrevistas" |
| ✗ No mechanism | "Tu CV tiene errores que no ves" |
| ✓ All three | "3 cambios para que el ATS sí lea tu CV" |
| ✓ All three | "El ATS descarta tu CV antes de que un humano lo vea" |
| ✓ All three | "Tu CV se ve bien. El filtro ATS no opina igual." |

**Preflight check:** slide 1 headline must contain at least one of: `CV`, `ATS`, `currículum`, `hoja de vida`, `perfil`, `LinkedIn`, `reclutador`, `filtro`, `entrevista`. If none present → preflight WARN "S1 hook may lack context object."

**Why it matters:** a stranger must know what this carousel is about in 2 seconds cold. No pronoun-without-antecedent, no punchline setups that require slide 2 to make sense, no mystery hooks where the subject is only revealed later.
