...
A demo that only demonstrates comes out plain. The mechanism is true but bloodless — it
shows a document changing, not a person getting seen. Direct every video on the
**Filtering Tunnel**: the candidate was always qualified; a dumb machine couldn't parse
them. Don't apologize for the applicant — indict the filter. The red flags are the
*machine's* failure to see a human, not the human's failure to be enough. Put the camera
on the candidate's side.

Five non-negotiables for any demo video (these are what separate a story from a feature tour):

1. **Open on the wound, not a feature.** First 5 seconds target one body sensation: the
   drop of relief when "it's not me" lands. Name the pain (40 CVs, zero replies) and the
   lie they believe (*no soy suficiente*), then crack it. Never open on a product label.
2. **Name a villain.** The filter/bot that rejects you before a human reads you. Make it
   visible (a red scan, a RECHAZADO stamp). The score is the machine's verdict, framed as
   the system's design — never the candidate's worth. Honesty constraint: the filter is
   mechanical, not evil. Don't imply malice; don't promise "they'll read you."
3. **Break the rhythm.** Identical proof beats are a monotone. Make one of them hesitate —
   a held breath, a pause, an asymmetry. That single off-grid choice is where the human
   directing hand becomes visible.
4. **Climax on the insight, not a caption.** Run the climax line through the paste test
   like any other copy. "Personaliza tu CV en segundos" fails — Rezi could say it. The
   cheat code ("esto es lo que hacen los que sí consiguen entrevista") cannot be copied.
5. **Close on the human, then the logo.** Land the consequence on the person first — their
   facts never changed, only their visibility did. The CTA comes after the loop closes.

Then ship it as a **hypothesis, not a truth.** Every video has a written falsifier
(e.g. lower save/share/CTR than the prior cut in an A/B) and gets killed or kept on data,
not on taste. Surety brings ruin.
